...
// automatically displays that notification. Only show manually for data-only
// messages (no notification block) so a single alert is shown.
...
self.addEventListener("activate", (e) => {
  // Clear any caches left over from previous versions of this SW.
  e.waitUntil(caches.keys().then((keys) => Promise.all(keys.map((k) => caches.delete(k)))).then(() => self.clients.claim()));
});

// No fetch interception — let the browser handle all caching normally.
